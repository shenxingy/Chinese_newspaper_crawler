        ...
        if parent:
            ...
            if parent:
                ...
                if subtitle:
                    ...
                    subtitle = ''
        data_list.append({'special_title': special_title, 'title': title, 'subtitle': subtitle, 'content': content, 'url': current_url, 'date': date})
...
def main():
    ...
    papers = pd.DataFrame(columns=['special_title', 'title', 'subtitle', 'content', 'url', 'date', 'ban'])
    for url in urls:
        news = get_news(url)
        papers = pd.concat([papers, news], ignore_index=True)
    papers.to_csv('XinMin.csv', index=False)
